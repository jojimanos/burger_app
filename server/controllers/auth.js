...
const { registerEmail, forgotPasswordEmail, orderEmail } = require('../helpers/email');
...
exports.register = (req, res) => {
  ...
  const { name, address, postalCode, birthDate, email, password } = req.body
  ...
  User.findOne(req.body.email).exec((err, user) => {
    ...
    const register = registerEmail(name, email, token)
...
exports.order = (req, res) => {
  ...
  const { lettuce, tomato, beef, cheese } = req.body
  //check if user exists in our db
  User.findOne(req.body.email).exec((err, user) => {
    if (user) {
      return res.status(400).json({
    ...
    // generate token with all fields
    const order = orderEmail(lettuce, tomato, beef, cheese)
    ...
    sendgrid
      ...
      .then((response) => {
        ...
        console.log(response[0].headers)
        res.json({
          message: `Your order will take about a minute.`
        })
      })
      .catch((error) => {
        console.error(error)
